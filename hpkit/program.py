...
docs/topics/formats.md). What this does is replace the source inside
...
def _source_record(b):
    ...
    So that is what is searched for. The records that wrap it are the ones
    ending exactly where it ends, and those are the lengths to fix when the
    text changes.

    Of the candidates, the largest one used to win outright, and for one
    file in a few hundred that is the wrong answer. A record that wraps the
    source starts eight bytes before it, so its payload begins with the
    source record's own header: four bytes of length, four of tag. If the
    low half of that length lands on <printable ASCII> 00 -- that is, if the
    inner length modulo 65536 falls in the printable range -- the wrapper
    reads as text too, it is bigger, and it wins. Then `read` gives back the
    header bytes with the source glued behind them, `write` patches the
    wrong lengths, and a file that was written correctly is refused because
    it does not read back. It is a coincidence of size, not a bad file: one
    character more or less in the source and it passes -- which is how it
    turned up, on a generated app source that changes length with its data.
    97 lengths in every 32,768 hit it; against the template this kit ships,
    32,737 characters is one of them.

    So the end is chosen by the largest candidate, and then, **among the
    candidates that end there, the innermost wins**: one that ends where
    another ends and starts before it does not sit beside it, it contains
    it. For a false candidate to turn up inside the text instead, four bytes
    of that text would have to spell the exact remaining length, and read as
    UTF-16LE ASCII the smallest value they can spell is 0x00200020 -- so it
    would take a container above 2 MB, twice the largest seen.
    ...
    end = max(cands, key=lambda c: c[1] - c[0])[1]
    off = max(o for o, e in cands if e == end)   # the innermost, not the wrapper
