...
It shows up in two places:
  ...
  - the COMPILED BLOCK the calculator puts before the source in programs
    that declare matrices (see docs/topics/formats.md);
...
docs/topics/formats.md; the hard part -- the number format -- is no
longer in the way.
...
def read_hpmat(data, strict=True):
    ...
    # A vector is rank 1, and then 1 and its number of elements, so it reads
    # as a matrix of one row. Measured 2026-09-11 by storing [1,2,3] in M1 on
    # the Virtual Calculator and reading the file back: 40 bytes, the two
    # words after the rank 1 and 3, the elements 1, 2 and 3. The one-element
    # vectors found in two mirrored calculators, where both words are 1,
    # agree with it. The writer always writes rank 2, so a vector written
    # again comes back a 1xn matrix.
