...
def hpmat_files():
    ...
    for calc in calculators():
        for p in sorted(glob.glob(os.path.join(calc, '*.hpmat'))):
            ...
            # 80 form. A vector, rank 1, is written back as a 1xn matrix. So
            # the bytes match exactly only for a rank-2 file of the written
            # type; everywhere else it is the numbers that have to survive.
            kind, rank = struct.unpack_from('<2xHI', data, 0)
            if kind == N.HPMAT_WRITE_TYPE and rank == 2:
                ...
                ok(again == m,
                   '%s: %dx%d, type %04X, rank %d, the numbers survive'
                   % (os.path.basename(p), len(m), len(m[0]), kind, rank))
    if not seen:
...
def real_block():
    ...
    for calc in calculators():
        for p in sorted(glob.glob(os.path.join(calc, '*.hpprgm'))):
            ...
            # A global that is not exported, and a function declared ahead of
            # its body, are in the block too: VaporHP carries tol, a plain
            # global, and toSIunit_p, a forward declaration. So everything
            # the program defines counts, read the way the linter reads it.
            from hpkit import lint
            declared |= lint.scan_names(src)[0]
