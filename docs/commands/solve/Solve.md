...
ran; the other two predate that round and were measured with the Function
...
**That makes it the exception to the phase's main rule** (emulator).
...
(emulator). Three commands in this phase read their app's own state rather
...
**A name clash waits in Phase 8** (HP help): the app variable `SOLVE` differs
from this only in case, and the suffix rule built for that is recorded in the
phase context.
